...
<h1>{{ $modo == 'editar' ? 'Editar Usuario' : 'Restablecer Contraseña' }}</h1>
@if ($errors->any())
...
@endif
<form action="{{ $modo == 'editar' ? route('UpdateUsuario', $user->id) : route('ResetPassword', $user->id) }}" method="POST">
@csrf
    @if ($modo == 'editar')
        @method('PUT')
